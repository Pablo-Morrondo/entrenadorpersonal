...
const SYNC='entrenador-v053-view-sync',core=read(CORE,{activities:[]});if(!localStorage.getItem(SYNC)&&(core.activities||[]).some(a=>a.source==='manual-recovery-v052')){write(SYNC,new Date().toISOString());setTimeout(()=>location.reload(),100)}
const ver=()=>{const d=$('#date');if(d)d.innerHTML=d.innerHTML.replace(/v0\.5(?:\.[12])?|v0\.4(?:\.2|\.3)?|v0\.3(?:\.2)?/,'v0.5.3');const v=$('.version');if(v)v.textContent='Entrenador Personal · v0.5.3'};ver();setTimeout(ver,800);
if('serviceWorker'in navigator)addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=053').then(r=>r.update()).catch(()=>{}));
})();
